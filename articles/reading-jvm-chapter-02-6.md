...
フレームとは，メソッドを実行するために必要な情報を格納するコンテナです。
例えば，ローカル変数やオペランド・スタック，メソッドの実行時定数プールへの参照，およびメソッドの呼び出し元に戻るための情報などが含まれます。
さらに，JVM の実装によっては，デバッグ情報などの追加の情報を持つ場合があります。

フレームは，JVMがメソッドを呼び出す際に作成され，メソッドの実行中に必要な情報を保持します。
フレームは「JVM スタック」というスタック上に積まれ，メソッドの呼び出しと修了に伴ってプッシュおよびポップされます（前記事参照）。
メソッドの実行が正常に終了しても，（例外が発生して）異常終了したとしても，フレームは例外なくスタックから取り除かれて破棄されます。
...
各フレームには専用の**ローカル変数の配列**があり，これにはメソッドの引数やローカル変数が格納されます。
ローカル変数とは，メソッド内の命令列が一時的に値を格納して，任意のタイミングでその値を参照できるようにするためのものです。

:::message
いつも Java プログラムを書いているときには，ローカル変数がどのように管理されているかを 意識することはあまりありません。
...
もしくは２つの連続した要素を組み合わせて `long` 型や `double` 型の値を表現できます。
...
long 型または double 型の値は，64 ビット（８バイト）幅を持つため，１つの要素（32 ビット）では格納できません。
そのため，これらの型の値を格納するには，**２つの連続した要素を使用**します。
なお，このような 64ビット幅を型を「カテゴリ 2」型と呼びます（それ以外の型は「カテゴリ 1」型と呼びます）。

カテゴリ２型の値を格納する場合，**最初の要素に下位 32 ビットを，次の要素に上位 32 ビットを格納**します。
...
:::message alert
このとき，２つ目のインデックス（この場合は `slot 1`）は，その値の一部を格納するために予約されているだけであり，**独立したローカル変数として使用することはできません**。
...
引数の数はメソッドの引数の数と，静的メソッドかインスタンス・メソッドかによって異なります。

- **静的メソッド**の場合：ローカル変数配列の `0` 番目の要素から順に，メソッドの引数が格納される
- **インスタンス・メソッド**の場合：ローカル変数配列の `0` 番目の要素には，そのメソッドが属するオブジェクト（Java で云うところの `this`）への参照が格納され，その後にメソッドの引数が順に格納される
...
これも，フレームの作成時に（メソッドのバイト・コードに埋め込まれた情報に基づいて）その最大サイズが決定され，実行時に拡大・縮小することはできません。
...
各命令には影響するオペランド・スタックの要素数が決まっており，命令の実行前にオペランド・スタックに十分な要素が存在する必要があります。
