...
一方で `double` 型の制御変数を使ったループでは，まず`dcmpg` 命令という命令で比較をしたあとに，`ifgt` 命令でジャンプをしています。
つまり，比較とジャンプが別々になっているのです。

なら，「`int` 型でも `icmpg` 命令と `ifgt` 命令で良いのでは？」と思うかもしれませんし、「`double` 型にも `if_dcmplt` 命令を用意すればいいのでは？」とも考えたくなります。

しかしここには，JVM の命令セット設計上の制約が関係しています。
