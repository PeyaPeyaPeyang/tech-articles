...
算術演算をする命令のオペランド（命令の引数）は，オペランド・スタックから取り出され，またその演算結果はオペランド・スタックに積まれます。
ですから，或る演算命令の演算結果を，**別の算術命令のオペランドとして**（ローカル変数にいれたりせずにとも）使用できます。

つまり，複数の算術演算を連続して行う場合，**ただ連ねるだけで（副次的に入れ子構造のようになって）その結果を得られるのです**。
