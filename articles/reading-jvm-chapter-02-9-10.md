...
例えば `void m(Object... args)` というシグニチャのメソッドがあったとしても，実際には `m(1, "abc", true)` のように呼び出すことができます。
（本来は `m(new Object[]{1, "abc", true})` と呼び出す必要があります。）
...
その中でのメソッドの呼び出しは，実行時に引数の型に応じて柔軟に変化する必要があるのです。
...
JVM における全ての例外は， `java/lang/Throwable` クラスを親に持つサブクラスによって表現されます。
例外がスローされると，直ちにそのメソッド内から制御が呼び出し元に戻されます。
このとき，例外をスローしたメソッドのフレームは破棄されます。
