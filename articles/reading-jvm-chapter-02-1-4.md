...
皆様大好き クラス・ファイル・フォーマット（*`class` file format*）と呼ばれる形式のことです。
この形式は JVM 系言語で書かれたプログラムをコンパイルした結果生成される，**バイトコード**(*Byte code*)という形式で表現されます。

バイトコードにはクラスやインタフェースの定義，メソッドやフィールドの情報，さらには定数プールなどが含まれます。
詳しくは第４章で解説します。

:::message
実は JVM が実行するのは**あくまでもこのバイトコード**であり，例えば  Java 言語のソースコード自体や，クラス・ファイルではありません。
...
Java などで記述されたコードは JVM が受け付ける機械語である **バイト・コード**にコンパイルされます。
これはアセンブラとよく似ており，JVM という仮想的なスタック・マシン（スタックを操作して計算を行う仮想マシン）に対して命令を発行します。

バイト・コードは完全なバイト列ですから，私が敢えて記事に書くならば `0xCAFEBABE000000450000…`となります。
映画・マトリックスの登場人物のように，その中の命令を私は直接読めませんし，たぶん皆様も一緒であると思います（もし読める方がいらっしゃいましたら，コメントでぜひ教えてください！）。
...
そこで私は バイト・コードを ASCII 文字列で表す `javap` のような記法で表しますから，どうぞ皆々様はご安心してお読みください。 

これは `javap -v Main` コマンドを実行した結果（抜粋）です：
![javap](/images/reading-jvm-chapter-02-1-4/javap.png)

`javap` は Java のクラス・ファイルを逆アセンブルして，バイト・コードを人間が読める形式で表示する（遊びがいのある）ツールです。
...
これらは例えば変数に格納したり，或いは引数として渡したり，メソッドからの戻り値として使用したりできます。

### 型チェック

JVM は，実行時に細かな**型チェックを行いません**。
型のチェックは（通常はコンパイラによって）既に行われていることを前提としています。 
そのために JVM は型の整合性を保証しないのです。
...
http://zenn.dev/peyang/articles/reading-jvm-chapter-02-11-1-4
...
オブジェクト型は常に**それを参照する型**である**参照型**として扱われ，これは JVM において特別な意味を持ちます。
参照型の値はオブジェクトへのポインタを表し，オブジェクトのメモリ上の位置を指します。
オブジェクトの直接的な操作はできませんが，一方で参照を通じてオブジェクトのフィールドやメソッドにアクセスできます（＝参照渡し）。
...
このようにして JVM はオブジェクト型を参照型として扱い，オブジェクトのメモリ上の位置を指すポインタを使用して操作します。
...
実は JVM の内部では `boolean` 型というのは（明示的に）存在しません。
代わりに内部的にはそれぞれ**整数型**の `0` と `1` として表現されます。
...
この情報は *StackMapTable* としてクラス・ファイルに格納され，JVM がコードを実行する際に，*Verification*（検証）を行うために使用されます。
...
そのために， Java SE 1.6 以降ではコンパイル時に *StackMapFrame* を計算し，クラス・ファイルに格納する必要があります。
