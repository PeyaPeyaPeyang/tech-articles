...
このように JVM は命令ごとにオペランドの型を明示的に区別します。
...
オブジェクト型は常に**参照型**として扱われ，JVM において特別な意味を持ちます。
...
このとき `doStuff` メソッドに渡される `array` 引数は `arr` の**参照を受け取ります**。
...
これら整数型のデフォルト値（初期化をしなかった場合に設定される値）は `0` です。`char` 型は `\u0000`（ヌル文字）で初期化されます。
...
なお，正の `0.0` と負の `-0.0` は大きさは同じですが，操作によって異なる値として扱われます。
例えば `1.0` を `0.0` で割ると `Infinity` が得られますが，`1.0` を `-0.0` で割ると `-Infinity` が得られます。
...
内部的にはそれぞれ整数型の `0` と `1` として表現されますが，JVM の命令セットでは `boolean` 型に特化した命令は存在しません。
そのため，`boolean` 型の値は `int` 型の値として扱われ，また操作します。

これに特化した命令は存在しませんが，配列の作成だけは，`newarray` 命令を使用して特別にサポートされています。
一方で配列への値の代入や取得は `bastore` や `baload` 命令を使用して行います。

### `returnAddress` 型

この型は，JVM の命令セットにおいて特別な意味を持つ型であり，`jsr`，`jsr_w`，`ret` 命令のみで使用されます。
これは `jsr`(Jump to Subroutine) 命令でサブルーチンにジャンプしたとき，`ret` 命令でサブルーチンから戻るためのジャンプ元アドレスを表します。
`returnAddress` 型は JVM の命令セットにおいて特別な意味を持つため，通常のプログラムでは直接使用されることはありません。
